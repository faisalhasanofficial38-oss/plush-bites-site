...
function Home() {
  ...
  return (
    <>
      {/* HERO */}
      <section className="relative min-h-[100svh] flex items-center pt-32 pb-16 sm:pb-20">
        ...
        <div className="relative mx-auto max-w-7xl w-full px-5 sm:px-8">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full glass-soft px-4 py-1.5 text-[11px] uppercase tracking-[0.25em] text-muted-foreground animate-fade-up">
              <span className="h-1.5 w-1.5 rounded-full bg-[var(--gold)] animate-pulse-glow"></span>
              {settings.hero_eyebrow}
            </div>
            <h1 className="mt-6 font-display text-[clamp(2.5rem,8vw,6rem)] font-semibold leading-[0.92] tracking-tight animate-slide-up stagger-1">
              {settings.hero_headline}
            </h1>
            <p className="mt-6 max-w-xl text-base sm:text-lg leading-relaxed text-muted-foreground animate-slide-up stagger-2">
              {settings.hero_description}
            </p>
            <div className="mt-10 flex flex-wrap items-center gap-3 animate-slide-up stagger-3">
              <Link to="/order" className="group relative rounded-full bg-gold-gradient px-8 py-4 text-sm font-semibold text-primary-foreground shadow-[var(--shadow-gold)] transition-all duration-300 hover:scale-[1.04] hover:shadow-[var(--shadow-glow-gold)] active:scale-[0.98]">
                Order now
                <span className="inline-block ml-1 transition-transform duration-300 group-hover:translate-x-1">→</span>
              </Link>
              <Link to="/menu" className="rounded-full glass-strong px-8 py-4 text-sm font-medium transition-all duration-300 hover:scale-[1.02] hover:shadow-[var(--shadow-glow-gold)]">View menu</Link>
              <a href={`tel:${settings.phone}`} className="rounded-full glass-soft px-8 py-4 text-sm font-medium transition-all duration-300 hover:scale-[1.02]">Call restaurant</a>
            </div>
            <dl className="mt-14 grid grid-cols-3 gap-3 sm:gap-4 max-w-lg animate-fade-up stagger-4">
              {[["4.0★", "889 Google reviews"], ["৳400–600", "Per person"], ["11:30–23", "Open daily"]].map(([k, v]) => (
                <div key={k} className="glass rounded-2xl p-4 text-center">
                  <dt className="font-display text-xl sm:text-2xl text-gradient-gold">{k}</dt>
                  <dd className="mt-1 text-[11px] text-muted-foreground">{v}</dd>
                </div>
              ))}
            </dl>
          </div>
        </div>
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 text-[10px] uppercase tracking-[0.4em] text-muted-foreground/60 animate-float-slow">
          <span>scroll</span>
          <span className="block h-8 w-px bg-gradient-to-b from-muted-foreground/40 to-transparent"></span>
        </div>
      </section>

      {/* MARQUEE */}
      <div className="relative overflow-hidden border-y border-border/60 py-7 bg-white/[0.02]">
        <div className="flex shrink-0 gap-12 whitespace-nowrap [animation:marquee_28s_linear_infinite]">
          {[..."Slow-cooked|Live Music|Craft Cocktails|Private Lounge|Late-night Bites|Reservations".split("|"), ..."Slow-cooked|Live Music|Craft Cocktails|Private Lounge|Late-night Bites|Reservations".split("|")].map((t, i) => (
            <span key={i} className="font-display text-2xl sm:text-3xl text-muted-foreground/60">
      ...
      <section className="relative py-24 sm:py-32">
        <div className="absolute inset-0 pointer-events-none bg-noise opacity-[0.08]"></div>
        <div className="relative">
          ...
          <div className="mx-auto mt-14 max-w-7xl px-5 sm:px-8 grid gap-6 md:grid-cols-2">
            {[
              ...
              <article key={s.k} className="glass hover-lift group relative overflow-hidden rounded-3xl p-8 sm:p-10 transition-all duration-700"
          ...
          <div className="mt-12 text-center">
            <Link to="/about" className="group inline-flex items-center gap-2 rounded-full glass-strong px-6 py-3 text-sm font-medium transition-all duration-300 hover:shadow-[var(--shadow-glow-gold)]">
              Learn our story
      ...
      <section className="relative py-24 sm:py-32 overflow-hidden">
        <div className="absolute top-0 right-0 w-96 h-96 rounded-full bg-[var(--gold)] opacity-[0.03] blur-[120px] pointer-events-none"></div>
            ...
            <article key={it.id} className="glass hover-lift group rounded-3xl overflow-hidden flex flex-col transition-all duration-500"
              ...
              <div className="relative aspect-[4/3] overflow-hidden">
                ...
                {(it.is_best_seller || it.is_featured) && (
                  <div className="absolute top-3 left-3 text-[10px] uppercase tracking-[0.25em] glass-strong rounded-full px-3 py-1 z-10">
        ...
        <div className="mx-auto mt-12 max-w-7xl px-5 sm:px-8 flex flex-wrap justify-center gap-3">
          <Link to="/menu" className="group rounded-full bg-gold-gradient px-8 py-4 text-sm font-semibold text-primary-foreground shadow-[var(--shadow-gold)] transition-all duration-300 hover:scale-[1.04] hover:shadow-[var(--shadow-glow-gold)]">
            Explore full menu
            <span className="inline-block ml-1 transition-transform duration-300 group-hover:translate-x-1">→</span>
          </Link>
          <Link to="/order" className="rounded-full glass-strong px-8 py-4 text-sm font-medium transition-all duration-300 hover:shadow-[var(--shadow-glow-gold)]">Reserve a table</Link>
        </div>
      </section>
      ...
      <section className="relative py-24 sm:py-32">
        <div className="mx-auto max-w-7xl px-5 sm:px-8">
          <div className="relative overflow-hidden rounded-[2.5rem] glass-strong p-8 sm:p-16 lg:p-20">
            ...
            <div className="relative grid lg:grid-cols-[1.4fr_1fr] gap-10 items-center">
              ...
              <div className="flex flex-col gap-3">
                <Link to="/order" className="group rounded-full bg-gold-gradient px-7 py-4 text-center text-sm font-semibold text-primary-foreground shadow-[var(--shadow-gold)] transition-all duration-300 hover:scale-[1.02] hover:shadow-[var(--shadow-glow-gold)]">
                  Reserve a table
                  <span className="inline-block ml-1 transition-transform duration-300 group-hover:translate-x-1">→</span>
                </Link>
                <a href={settings.foodpanda_url} target="_blank" rel="noreferrer" className="rounded-full glass px-7 py-4 text-center text-sm font-medium transition-all duration-300 hover:scale-[1.02] hover:shadow-[var(--shadow-glow-gold)]">Order on foodpanda</a>
                <a href={`tel:${settings.phone}`} className="rounded-full glass-soft px-7 py-4 text-center text-sm font-medium transition-all duration-300 hover:scale-[1.02]">Call {settings.phone}</a>
              </div>
            </div>
          </div>
        </div>
