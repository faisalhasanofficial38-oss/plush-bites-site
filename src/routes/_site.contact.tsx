    ...
    <>
      ...
      <section className="mx-auto max-w-7xl px-5 sm:px-8 pb-20 grid gap-8 lg:grid-cols-2">
        <form
          ...
          className="glass-strong rounded-3xl p-6 sm:p-10 grid gap-5"
        >
          ...
          <button type="submit" className="mt-2 rounded-full bg-gold-gradient px-7 py-4 text-sm font-semibold text-primary-foreground shadow-[var(--shadow-gold)] transition-all duration-300 hover:scale-[1.02] hover:shadow-[var(--shadow-glow-gold)] active:scale-[0.98]">
            Send via WhatsApp
          </button>
          {sent && <p className="text-sm text-[var(--gold)] text-center animate-fade-in">Opened in WhatsApp — send the message to confirm.</p>}
        </form>
        ...
        <div className="glass-strong rounded-3xl overflow-hidden flex flex-col">
          <div className="p-8 sm:p-10 space-y-4">
            ...
            <a href="https://maps.app.goo.gl/xKQkJ8mMpbg97x4C9" target="_blank" rel="noreferrer"
              className="group inline-flex items-center gap-2 rounded-full bg-gold-gradient px-6 py-3 text-sm font-semibold text-primary-foreground shadow-[var(--shadow-gold)] transition-all duration-300 hover:shadow-[var(--shadow-glow-gold)]">
              Get directions
...
function ContactCard({ icon, label, value, href }: { icon: string; label: string; value: string; href: string }) {
  return (
    <a href={href} target={href.startsWith("http") ? "_blank" : undefined} rel="noreferrer"
      className="glass rounded-3xl p-8 flex items-center gap-5 group transition-all duration-500 hover:shadow-[var(--shadow-gold)] hover:border-[var(--gold)]/40"
      style={{ animation: "fade-up 0.9s cubic-bezier(0.22, 1, 0.36, 1) both" }}>
      <span className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-gold-gradient text-2xl shadow-[var(--shadow-gold)]">{icon}</span>
      <div className="min-w-0">
        <div className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">{label}</div>
        <div className="mt-1 font-display text-lg truncate group-hover:text-gradient-gold transition-all duration-300">{value}</div>
      </div>
    </a>
  );
}
...
function Field({ name, label, placeholder, type = "text", required, textarea }: { name: string; label: string; placeholder?: string; type?: string; required?: boolean; textarea?: boolean }) {
  ...
  return (
    <label className="block">
      ...
      {textarea ? (
        <textarea name={name} placeholder={placeholder} required={required} rows={4} className={`${base} resize-none`} />
      ) : (
